(() => {
  ...
  const SETTINGS = {
    overlayFade: .4,
    cornerFade: .3,
    pickup: { soundFade: .8, popupFade: .6, trailFade: .35 },
    ...
    sound: {
      pingUrl: "https://cdn.prod.website-files.com/6292053974c40677f3ec8971/6a9bc5637521c66b5c85d17d_Reception-Ping.mp3",
      pingVolume: .5,
      typingUrl: "https://cdn.prod.website-files.com/6292053974c40677f3ec8971/6a9d71768425ed54cd3b20cb_Reception-Typing.mp3",
      typingVolume: .35,
      streetUrl: "https://cdn.prod.website-files.com/6292053974c40677f3ec8971/6a9d7944f79d96f6c8c13b37_Reception-StreetNoise.mp3",
      streetVolume: .4,
      streetFadeDuration: 4,
      ringUrl: "https://cdn.prod.website-files.com/6292053974c40677f3ec8971/6a9e6cc17058d12cadbd78fb_Reception-Ring.mp3",
      ringVolume: .6,
      ambientUrl: "https://cdn.prod.website-files.com/6292053974c40677f3ec8971/6aa2631c7996b4480bad3d55_Reception-AmbientEnvironment.mp3"
    },
  ...
  function createPhoneZone(options) {
    const button = document.querySelector(".phone-btn");
  ...
  function createSoundManager(options) {
    ...
    for (const asset of Object.values(assets)) {
      Object.assign(asset, { bytesPromise: null, decodePromise: null, buffer: null, output: null, source: null });
    }
  ...
  function createRandomNotifications(options, sound, phoneZone) {
    const wrapper = document.querySelector("[data-random-wrapper]");
  ...
  function createPhone(options, sound) {
    const button = document.querySelector(".phone-btn");
    if (!button) return { start() {} };

    const label = button.parentElement?.querySelector(".phone-btn__label") || document.querySelector(".phone-btn__label");
    ...
    function updateVolume() {
      ...
      if (!onScreen || !pointer && !focused) {
        sound.setBackgroundLevel(1, fade);
        return;
      }
      if (focused) {
        sound.setBackgroundLevel(0, fade);
        return;
      }

      const t = clamp(distance / radius, 0, 1);
      sound.setBackgroundLevel(t * t * (3 - 2 * t), fade);
    }
  ...
  function setupPage() {
    ...
    const overlay = document.querySelector(".sound-wrap");
    const mainButton = overlay?.querySelector(".sound-wrap__btn:not(.sound-wrap__btn--corner)");
    const cornerButton = document.querySelector(".sound-wrap__btn--corner");
    const phoneButton = document.querySelector(".phone-btn");
    const popup = document.querySelector(".genius-ambient-popup");
    ...
    const phone = createPhone(SETTINGS.phone, sound);
    ...
    function onPickup(event) {
      ...
      gsap.to(popup, {
        ...
        onComplete() {
          ...
          gsap.delayedCall(SETTINGS.ambient.delay, () => {
            if (video) {
