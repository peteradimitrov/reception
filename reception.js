(() => {
  ...
  function createRandomNotifications(options, sound, phoneZone) {
    ...
    function buildLayout() {
      ...
      items.forEach((item, i) => {
        ...
        const allowedOverlap =
          .05 + .45 * density * Math.pow(progress, 1.2);
    ...
    function nextDelay() {
      ...
      const progress = clamp(
        elapsed / Math.max(.1, options.rampDuration),
        0,
        1
      );
      ...
      const curve = Math.pow(
        progress,
        Math.max(.01, options.acceleration)
      );
  ...
  function createPhone(options, sound) {
    ...
    const label =
      ...
      document.querySelector(".phone-btn__label");
  ...
  function setupPage() {
    ...
    const cornerButton = document.querySelector(
